...
| F25 | P1 | in-progress | **Beheer restructure**: admin screens organized into main categories with a **navigation bar on the right** (club, theme, user management, teams, location management, schedule management, system) instead of one long list of cards; system category visible to instance admins only (F26). |
...
| F28 | P3 | planned | **KNVB season playsheets import** (instance level): load the season playsheets the KNVB publishes (PDF, parseable) so clubs/teams can adopt the official season structure. |
