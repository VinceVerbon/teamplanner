...
| F29 | P2 | in-progress | **Speeldagenkalender -> schedule**: schedule planning consumes the team's selected kalender column (F28). The team page Wedstrijden card gains **'uit de speeldagenkalender'**: kalender days are classified by cell value (**speeldag** - WD*/Fase/Week N/Beker*/round numbers/finals -> preselected; **reserve** - Inhaal/Uitwijk/combined '/'-values -> listed unchecked; Vrij/NC/'-'/empty -> not offered) and shown with date, label and remark (preview-before-load, mirrors F21); selected weekends import as **placeholder matches** (opponent 'Speeldag <celwaarde>', locatie 'n.t.b.', chosen start time + 2h, weekend rows use their first day) with **idempotent re-import** via externalUid `kalender:<date>` per team; placeholders coexist with later real F21 imports and are removed manually. Bundled F26 leftover: **week numbers rendered** in the schedule views per the instance weekNumbering setting (iso = weeks start Monday / us = weeks start Sunday), pure shared week-number util. |
...
| ID | P | Status | Feature |
|----|---|--------|---------|
| F18 | P2 | in-progress | **Production deploy stack**: `deploy/` compose + Dockerfile + Postgres per WhereLog pattern; `docs/deploy-to-<host>.md` written at first deploy. Built: `deploy/compose.yaml` (tp-app + tp-db Postgres 17; named `tp-pgdata` volume as the single backup surface, DB not host-exposed, app starts only after db healthcheck, commented Traefik block for the proxy hosts), multistage `deploy/app/Dockerfile` (node:24-alpine, non-root uid 10001, ships the Drizzle migrations at the startup-migration path, healthchecks `/api/healthz`), `deploy/.env.example` (env contract incl. Gmail-SMTP MVP mail decision) + root `.dockerignore`; config validated by `docker compose config` and the test-set. **DEPLOYED 2026-08-03: live on dexter as https://teamplanner.syquens.com** (Traefik overlay `deploy/traefik.dexter.yaml`, DNS via GoDaddy API, secrets on host + 1Password `teamplanner dexter prod`; SMTP empty pending the Gmail open issue) - see `docs/deploy-to-dexter.md`. |
| F31 | P1 | in-progress | **Secure first-run (F22 hardening)** - BUILT 2026-08-06: bootstrap routes gated on `BOOTSTRAP_TOKEN` (timing-safe; 404 when unset, so a completed install keeps no bootstrap surface) + per-IP rate limits; seed is `password: null` (legacy scrypt('') seeds still accepted by `setBootstrapPassword`, real passwords never); advertising removed (/login alert gone, status = bare boolean behind the token, /setup-admin only via the installation link); `/api/me/password` rate-limited per user; `ensureBootstrapAdmin` idempotent on `isBootstrapAdmin`; `AUTH_DISABLE_SIGNUP` env closes self-registration for invite-only instances (default open). Original finding: security review of the live dexter deploy 2026-08-03 rated the current bootstrap a CRITICAL unauthenticated admin-takeover window: `POST /api/bootstrap/password` has no session, token, CSRF or rate limit, `/setup-admin` is public, and `GET /api/bootstrap/status` is a free oracle that the login page even advertises; the only "gate" is DB state, so on any fresh public deploy the first caller to reach the port becomes instance admin. Also: the seeded admin stores a real scrypt hash of the empty string, so `sign-in/email` with `password: ''` authenticates (the `account.password` column is already nullable). Fix: (a) gate first-run on a deploy-time secret (`BOOTSTRAP_TOKEN` env compared timing-safe; route refuses entirely when unset) or a one-shot token printed to the container log; (b) seed with `password: null` so normal sign-in can never authenticate the bootstrap admin; (c) stop advertising - drop the `/login` alert and reduce `bootstrap/status` to a boolean behind the same token; (d) rate-limit `/api/me/password` (server-side `auth.api.changePassword()` bypasses better-auth's router limiter). Decide alongside: `disableSignUp` for invite-only, and make `ensureBootstrapAdmin()` idempotent on `isBootstrapAdmin` rather than "zero users" (a stranger's signup currently suppresses seeding entirely, with no recovery path in code). |
